...
const screenForNames = (arr) => {
  // Solution code here...
  let newArr= arr.map((val) => val.match(/^(Mr|Mrs|Ms|Dr|)\..[A-Za-z]+$/g)).filter((item) => item).flat();
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map((val) => val[0].toUpperCase() + val.slice(1));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.reduce((alpha, beta) => alpha + (beta.mass > 77 ? " - " + beta.name : ""), "").slice(3);
};
