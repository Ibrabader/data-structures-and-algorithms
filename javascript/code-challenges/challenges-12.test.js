...
const replaceZeros = (string) => {
  ...
  let response = string.replace(/0/g, "zero");
  return response;
}
...
const validatePin = (pin) => {
  // Solution code here...
  let numberOfPin = /^(\d{4}|^\d)$/;
return numberOfPin.test(pin);
};
...
const validateWord = (word) => {
  // Solution code here...
  let respnse2 = /\b[a-zA-Z]{5,10}\b/;
  
  return  respnse2.test(word);
};
...
const hasNumber = (string) => {
  // Solution code here...
  let numStr = /[a-zA-z]+[0-9]+/;

  return numStr.test(string);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let nPhone = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  
  return nPhone.test(phoneNumber);
};
